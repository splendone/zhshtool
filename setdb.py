...
class Ui_Dialog(object):
    ...
    def showTables(self):
        ...
        for i in range(0, length-1):
            itemi=data[i]
            tableName=itemi[0]
            self.listWidgetTables.addItem(tableName)

        self.listWidgetTables.show()

    def showFeilds(self):
        sql = "desc {}".format(self.listWidgetTables.currentItem().text())
        r= cursor.execute(sql)
        data = r.fetchall()
        length=len(data)
        if length:
        ...
        for i in range(0, length-1):
            itemi=data[i]
            tableName=itemi[0]
            self.listWidgetFeilds.addItem(tableName)
    ...
    def retranslateUi(self, Dialog):
        ...
        self.lineEditUserName.setText(_translate("Dialog", "xhf", None))
        self.lineEditPassword.setText(_translate("Dialog", "asdw", None))
        self.lineEditDatabase.setText(_translate("Dialog", "orcl", None))
        self.labelPort.setText(_translate("Dialog", "PORT", None))
        self.lineEditPort.setText(_translate("Dialog", "1521", None))
